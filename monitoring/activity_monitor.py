...
function
Dashboard({activeSession, onSessionUpdate})
{
    const[sessionName, setSessionName] = useState('');
const[activities, setActivities] = useState([]);
const[stats, setStats] = useState(null);
const[monitorProc, setMonitorProc] = useState(null);
...
const
startMonitor = () = > {
if (monitorProc | | !activeSession) return;
const
proc = spawn('python', ['monitoring/activity_monitor.py', '--session-id', activeSession.id]);
setMonitorProc(proc);
...
proc.stderr.on('data', (data) = > console.error('Monitor error:', data.toString()));
console.log('Started phone monitor');
};
...
setMonitorProc(null);
setIsMonitoring(false);
console.log('Stopped phone monitor');
}
};
...
return '';
return new
Date(dateString).toLocaleTimeString();
};
...
< / button >
    ...
    < button className={`button ${isMonitoring ? 'button-danger': 'button-primary'}`} onClick={isMonitoring ? stopMonitor: startMonitor} >
    {isMonitoring ? 'Stop Webcam Monitor': 'Start Webcam Monitor'}
    < / button >
    < p > {isMonitoring ? 'Monitoring phone... (check external window)': ''} < / p >
    < / div >
)}
< / div >
)}
...
Score < / h3 >
          < div
className = "value" > {stats.productivityScore?.toFixed(1)} % < / div >
                                                                  < div
className = "label" > Efficiency < / div >
                                     < / div >
                                         < div
className = "stat-card" >
            < h3 > Distraction
Time < / h3 >
         < div
className = "value" > {formatDuration(stats.totalDistractedTimeSeconds)} < / div >
                                                                             < div
className = "label" > Today < / div >
                                < / div >
                                    < div
...
Score < / h3 >
          < div
className = "value" > {stats.distractionScore?.toFixed(1)} % < / div >
                                                                 < div
className = "label" > Needs
Improvement < / div >
                < / div >
                    < / div >
)}
